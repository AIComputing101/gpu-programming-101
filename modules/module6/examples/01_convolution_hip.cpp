 ...
 * Example 1: Convolution Algorithms (HIP Implementation)
 * 
 * This example demonstrates various convolution implementations using HIP:
...
/**
 * Optimized 1D convolution with shared memory (LDS)
 */
...
/**
 * Optimized 2D convolution with Local Data Store (LDS) tiling
 */
__global__ void conv2d_shared(float *input, float *kernel, float *output,
    ...
    // Load input data into LDS with halo
...
// =============================================================================
// Wavefront-optimized functions (AMD-specific)
// =============================================================================

/**
 * Wavefront-optimized reduction for kernel normalization
 */
__device__ float wavefront_reduce_sum(float val) {
    #pragma unroll
    for (int offset = 32; offset > 0; offset >>= 1) {
        val += __shfl_down(val, offset, 64); // AMD wavefront is 64 threads
    }
    return val;
}
...
void benchmark_conv1d() {
    ...
    HIP_CHECK(hipMalloc(&d_kernel, kernel_size * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_output, input_size * sizeof(float)));
    
    // Copy data to device
    HIP_CHECK(hipMemcpy(d_input, h_input, input_size * sizeof(float), hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_kernel, h_kernel, kernel_size * sizeof(float), hipMemcpyHostToDevice));
    ...
    for (int i = 0; i < num_iterations; i++) {
        hipLaunchKernelGGL(conv1d_naive, grid_size, block_size, 0, 0, 
                          d_input, d_kernel, d_output, input_size, kernel_size);
    }
    HIP_CHECK(hipDeviceSynchronize());
    double naive_time = timer.stop() / num_iterations;
    
    HIP_CHECK(hipMemcpy(h_output_naive, d_output, input_size * sizeof(float), hipMemcpyDeviceToHost));
    ...
    timer.start();
    for (int i = 0; i < num_iterations; i++) {
        hipLaunchKernelGGL(conv1d_shared, grid_size, block_size, shared_mem_size, 0,
                          d_input, d_kernel, d_output, input_size, kernel_size);
    }
    HIP_CHECK(hipDeviceSynchronize());
    double shared_time = timer.stop() / num_iterations;
    
    HIP_CHECK(hipMemcpy(h_output_shared, d_output, input_size * sizeof(float), hipMemcpyDeviceToHost));
...
void benchmark_conv2d() {
    ...
    HIP_CHECK(hipMalloc(&d_kernel, kernel_area * sizeof(float)));
    ...
    HIP_CHECK(hipMemcpy(d_kernel, h_kernel, kernel_area * sizeof(float), hipMemcpyHostToDevice));
    ...
    timer.start();
    for (int i = 0; i < num_iterations; i++) {
        hipLaunchKernelGGL(conv2d_naive, grid_size, block_size, 0, 0,
                          d_input, d_kernel, d_output, width, height, kernel_size);
    }
    HIP_CHECK(hipDeviceSynchronize());
    double naive_time = timer.stop() / num_iterations;
    
    HIP_CHECK(hipMemcpy(h_output_naive, d_output, image_size * sizeof(float), hipMemcpyDeviceToHost));
    ...
    timer.start();
    for (int i = 0; i < num_iterations; i++) {
        hipLaunchKernelGGL(conv2d_shared, tile_grid, block_size, shared_mem_size, 0,
                          d_input, d_kernel, d_output, width, height, kernel_size);
    }
    HIP_CHECK(hipDeviceSynchronize());
    double shared_time = timer.stop() / num_iterations;
    
    HIP_CHECK(hipMemcpy(h_output_shared, d_output, image_size * sizeof(float), hipMemcpyDeviceToHost));
...
void benchmark_separable_conv() {
    ...
    // Copy data to device
    HIP_CHECK(hipMemcpy(d_input, h_input, image_size * sizeof(float), hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_kernel, h_kernel_1d, kernel_size * sizeof(float), hipMemcpyHostToDevice));
    ...
    for (int i = 0; i < num_iterations; i++) {
        // Horizontal pass
        hipLaunchKernelGGL(conv_separable_horizontal, grid_size, block_size, 0, 0,
                          d_input, d_kernel, d_temp, width, height, kernel_size);
        // Vertical pass
        hipLaunchKernelGGL(conv_separable_vertical, grid_size, block_size, 0, 0,
                          d_temp, d_kernel, d_output, width, height, kernel_size);
    }
    HIP_CHECK(hipDeviceSynchronize());
    double separable_time = timer.stop() / num_iterations;
    
    HIP_CHECK(hipMemcpy(h_output_separable, d_output, image_size * sizeof(float), hipMemcpyDeviceToHost));
 ...
 */
int main() {
    printf("Module 6: Convolution Algorithms (HIP Implementation)\n");
    printf("====================================================\n\n");
